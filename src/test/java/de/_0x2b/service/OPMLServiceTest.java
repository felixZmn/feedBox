...
class OPMLServiceTest {
  ...
  void exportOpml_returnsXmlString() {
    Feed unfiledFeed = new Feed(1, null, "A",
  ...
  @Test
  void createOPML_buildsBodyWithUnfiledFeedsDirectlyAndFoldersNested() throws Exception {
    Feed unfiledFeed = new Feed(1, null, "UnfiledFeed",
        java.net.URI.create("https://unfiled.example"),
        java.net.URI.create("https://unfiled.example/rss"));
    ...
    FolderTree folderTree = new FolderTree(List.of(tech), List.of(unfiledFeed));
    Document doc = sut.createOPML(folderTree);
    ...
    // body should contain:
    // - 1 feed outline directly (UnfiledFeed)
    // - 1 folder outline (Tech) containing its feed
    NodeList outlines = body.getElementsByTagName("outline");

    // There will be 3 outlines total in the subtree: UnfiledFeed, Tech(folder),
    // HN(feed inside Tech)
    assertEquals(3, outlines.getLength());

    // Assert unfiled feed is present somewhere and is type=rss
    assertTrue(containsFeedOutline(doc, "UnfiledFeed", "https://unfiled.example/rss", "https://unfiled.example"));
  ...
  void documentToString_outputsIndentedXml() throws Exception {
    FolderTree folderTree = new FolderTree(List.of(), List.of());
    Document doc = sut.createOPML(folderTree);
  ...
  private static boolean containsFeedOutline(Document doc, String name, String xmlUrl, String htmlUrl) {
    ...
    for (int i = 0; i < outlines.getLength(); i++) {
      Element el = (Element) outlines.item(i);
      if (!"rss".equals(el.getAttribute("type")))
        continue;
