...
import de._0x2b.service.HTTPSService;
...
import static de._0x2b.service.IconService.getCharsetFromContentType;
...
public class IconServiceTest {
    ...
    void getCharsetFromContentTypeTest() {
        ...
        assertEquals(StandardCharsets.UTF_8,
                getCharsetFromContentType("multipart/form-data; boundary=ExampleBoundaryString"));
        assertEquals(StandardCharsets.UTF_8,
                getCharsetFromContentType("multipart/form-data; boundary=ExampleBoundaryString; charset=utf-8"));
        assertEquals(StandardCharsets.US_ASCII,
                getCharsetFromContentType("Content-Type: application/xml; charset=US-ASCII"));
        assertEquals(StandardCharsets.UTF_8,
                getCharsetFromContentType("Content-Type: text/plain; charset=LATIN-DUMMY"));
    }
    ...
    void findOneByFeedTest() throws IOException {
        ...
        var defaultIcon = new Icon(-1, -1, getClass().getResourceAsStream("/static/icons/rss.svg").readAllBytes(),
    ...
    void findIconTest() {
        ...
        when(feed.getFeedUrl()).thenReturn(uri);
        ...
        when(htmlResponse.headers()).thenReturn(
        ...
        when(favResponse.headers())
                .thenReturn(HttpHeaders.of(Map.of("content-type", List.of("image/x-icon")), (k, v) -> true));
        ...
        when(httpsService.fetchAsBytes(eq(URI.create("https://example.org/test.ico"))))
                .thenReturn(Optional.of(favResponse));
